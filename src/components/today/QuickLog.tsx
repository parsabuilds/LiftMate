...
export function QuickLog() {
  ...
  // Show the saved display if weight exists in DB and user is not editing
  const showSavedState = hasSavedWeight && !isEditing;
    ...
    <div className="bg-card/60 border border-white/[0.06] rounded-2xl p-5 backdrop-blur-sm">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {showSavedState ? (
            /* Saved state — show the logged weight and an Edit button */
            <div className="flex items-center gap-3">
              <div className="flex-1 flex items-center gap-3 bg-bg/60 border border-primary/20 rounded-xl px-4 py-2.5 min-h-[44px]">
                ...
                <span className="text-text text-sm font-semibold">{dailyLog.weight} lbs</span>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#22C55E" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="ml-auto shrink-0">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              </div>
              <button
                onClick={handleEditWeight}
                className="min-h-[44px] px-4 bg-white/[0.06] hover:bg-white/[0.1] border border-white/[0.08] rounded-xl text-muted hover:text-text text-sm font-semibold transition-all"
              >
                Edit
              </button>
            </div>
          ) : (
            /* Input state — type weight and save */
            <div className="flex items-center gap-3">
